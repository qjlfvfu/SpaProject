...
class HabitForm(forms.ModelForm):
    ...
    class Meta:
        model = Habit
        fields = [
            'name', 'action', 'description', 'place', 'time',
            'planned_duration', 'period', 'is_pleasant', 'is_public',
            'reward', 'related_habit'
        ]
        widgets = {
            ...
            'planned_duration': forms.TextInput(attrs={
                'class': 'form-control',
            ...
            'is_pleasant': forms.Select(attrs={
                'class': 'form-select'
            }, choices=[(True, 'Да'), (False, 'Нет')]),
            'is_public': forms.Select(attrs={
                'class': 'form-select'
            ...
            'related_habit': forms.Select(attrs={
                'class': 'form-select'
            }),
        }
        labels = {
    ...
    def __init__(self, *args, **kwargs):
        ...
        if user:
            ...
            if self.instance and self.instance.pk:
                self.fields['related_habit'].queryset = self.fields['related_habit'].queryset.exclude(
                    pk=self.instance.pk
                )
    ...
    def clean_planned_duration(self):
        ...
        if duration and duration.total_seconds() > 120:
            raise forms.ValidationError('Время выполнения не должно превышать 120 секунд (2 минуты)')
        return duration
...
class TrackerForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            'actual_duration': forms.TextInput(attrs={
                'class': 'form-control',
                'placeholder': 'HH:MM:SS (например, 00:02:00)'
            }),
