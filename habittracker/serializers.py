...
class HabitSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Habit
        fields = [
            'id', 'name', 'action', 'description', 'place', 'time',
            'planned_duration', 'period', 'is_pleasant', 'is_public',
            'reward', 'related_habit', 'last_sent'
        ]
    ...
    def validate(self, data):
        ...
        if reward and related_habit:
            raise serializers.ValidationError("Нельзя заполнять одновременно награду и связанную привычку")
...
class TrackerSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Tracker
        fields = ['id', 'habit', 'habit_name', 'status', 'actual_duration', 'completed_date', 'created_at']
